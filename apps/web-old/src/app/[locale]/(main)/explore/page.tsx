...
import { Search, BadgeCheck, Heart, MessageCircle, MapPin, Bed, Bath, Maximize, Users, Crown, Medal, Building2, Home, TrendingUp } from 'lucide-react'
...
const topAreas = [
  { id: 'new-cairo', name: 'القاهرة الجديدة', count: 12500 },
  { id: 'october', name: '6 أكتوبر', count: 8700 },
  { id: 'zayed', name: 'الشيخ زايد', count: 6300 },
  { id: 'nasr-city', name: 'مدينة نصر', count: 5100 },
  { id: 'maadi', name: 'المعادي', count: 3800 },
  { id: 'heliopolis', name: 'مصر الجديدة', count: 2900 },
]
...
export default function ExplorePage() {
  ...
  return (
    <>
      ...
      {/* Hero Search */}
      <div className="bg-gradient-to-b from-primary/5 to-background px-4 pt-6 pb-4 border-b border-border">
        <h1 className="text-xl font-bold text-foreground mb-1">ابحث عن عقارك</h1>
        <p className="text-sm text-muted-foreground mb-4">أكثر من ١٥٠,٠٠٠ عقار في مصر</p>
        <div className="relative">
          <Search className="absolute start-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
          <input
            type="text"
            placeholder="ابحث بالمنطقة، النوع، أو الكلمة..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full h-12 ps-12 pe-4 rounded-xl bg-card border border-border focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-foreground placeholder:text-muted-foreground shadow-sm"
          />
        </div>
        {/* Quick actions */}
        <div className="flex gap-3 mt-4">
          <Link href="/listings?purpose=sale" className="flex-1 flex items-center gap-2 p-3 rounded-xl bg-card border border-border hover:border-primary/30 transition-colors">
            <div className="w-9 h-9 rounded-lg bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
              <Home className="w-4.5 h-4.5 text-green-600" />
            </div>
            <div>
              <p className="text-sm font-medium text-foreground">شراء</p>
              <p className="text-[11px] text-muted-foreground">عقارات للبيع</p>
            </div>
          </Link>
          <Link href="/listings?purpose=rent" className="flex-1 flex items-center gap-2 p-3 rounded-xl bg-card border border-border hover:border-primary/30 transition-colors">
            <div className="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
              <Building2 className="w-4.5 h-4.5 text-blue-600" />
            </div>
            <div>
              <p className="text-sm font-medium text-foreground">إيجار</p>
              <p className="text-[11px] text-muted-foreground">عقارات للإيجار</p>
            </div>
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-[680px] mx-auto pb-20">
        ...
        <section className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/listings" className="text-xs text-primary font-medium">عرض الكل</Link>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {topAreas.map((area) => (
              <Link
                key={area.id}
                href={`/listings?city=${area.id}`}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground">{area.count.toLocaleString('ar-EG')} عقار</p>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="flex gap-3 overflow-x-auto scrollbar-hide -mx-4 px-4">
            {mockUsers.map((user) => (
              ...
              >
                ...
                <Button size="sm" className="w-full bg-blue-600 hover:bg-blue-700 text-white" onClick={(e) => e.preventDefault()}>
                  متابعة
                </Button>
              </Link>
            ))}
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <Button
                    ...
                    className={`w-full ${!group.isJoined ? 'bg-green-600 hover:bg-green-700 text-white' : ''}`}
          ...
          <div className="space-y-3">
            {mockListings.slice(0, 3).map((listing) => (
              <Link
                ...
                className="flex gap-3 p-3 rounded-xl bg-card border border-border hover:border-primary/30 transition-colors"
              >
                <div className="relative w-24 h-24 rounded-lg overflow-hidden flex-shrink-0">
                  <Image
                    ...
                    className="object-cover"
                  />
                  <div className="absolute top-1 start-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-green-600 text-white">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><Maximize className="w-3 h-3" />{listing.specs.area} م²</span>
                    <span className="flex items-center gap-1"><Bed className="w-3 h-3" />{listing.specs.bedrooms}</span>
                    <span className="flex items-center gap-1"><Bath className="w-3 h-3" />{listing.specs.bathrooms}</span>
                  </div>
                </div>
              </Link>
            ))}
