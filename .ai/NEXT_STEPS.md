...
| **Testing & QA** | **85** | e2e (19) + **182 API unit** + **62 web** (logic + mock data layer + component renders + a real ReportButton interaction) + a CI coverage floor |
...
- [x] **BUG-1 · Banned users can still log in** 🐛 ✅ (June 25) — added `bannedAt` guards in **three** places: `jwt.strategy` (every request → 401 `ACCOUNT_BANNED`, even with a live token), `verifyOtp` (re-login → 403 with the ban reason), and `refresh` (→ 401). On ban, `AdminUsersService` now calls `AuthService.revokeAllSessions` (deletes the `rt:userId:*` refresh keys), and the public listings `list`/`findOne` exclude banned owners (`owner: { bannedAt: null }`). **e2e proves it:** admin ban → live session 401 → re-login 403 `ACCOUNT_BANNED`. 182 unit + **20 e2e** green.
