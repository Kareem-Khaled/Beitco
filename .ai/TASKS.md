...
- **Web component tests (TEST-3, first pass)** — stood up `@testing-library/react` + jsdom (own `vitest.config.ts` + `src/test/setup.ts` with jest-dom + a matchMedia stub), then **+21 tests** (31→52): the flag-aware **mock data layer** (`store.test.ts` — browse published, admin users/listings/content/analytics filters + takedown/restore round-trips) and **component renders** (`TrustBadge.test.tsx` — tone branches + EmptyState Arabic content). Next: `property.$id` actions + the wizard (need Router/Query wrappers).
